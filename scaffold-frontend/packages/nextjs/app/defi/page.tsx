...
export default function DeFiPage() {
  ...
  // Mock data - will be replaced with real smart contract data
  ...
  const handleStake = () => {
    console.log("Staking:", stakeAmount, "for", stakeDuration, "days");
  ...
  const estimateMonthlyYield = () => {
    ...
    return (amount * (currentAPR / 100)) / 12;
  };
  ...
  return (
    <div className="min-h-screen bg-[#041329] text-on-surface font-body">
      ...
      <main className="pt-24 pb-32 px-6 max-w-5xl mx-auto">
        ...
        {/* Pool Dominance */}
        <section className="mb-12 p-6 bg-surface-container-low rounded-xl border border-white/5">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          <div className="space-y-6">
            ...
            <div className="space-y-4">
              {stakingHistory.map((item, idx) => (
                <div
                  key={idx}
                  className="flex items-center justify-between p-4 bg-surface-container/50 rounded-xl border border-white/5"
                >
                  <div className="flex items-center gap-4">
                    <div
                      className={`w-10 h-10 rounded-full flex items-center justify-center ${
                        item.type === "deposit" ? "bg-primary/10" : "bg-tertiary/10"
                      }`}
                    >
                      <span
                        className={`material-symbols-outlined text-sm ${
                          item.type === "deposit" ? "text-primary" : "text-tertiary"
                        }`}
                      >
                  ...
                  <div className="text-right">
                    <p
                      className={`font-headline font-bold ${
                        item.type === "deposit" ? "text-error" : "text-primary"
                      }`}
                    >
          ...
          {/* Stake Assets Panel */}
          <div className="bg-surface-container-high p-6 rounded-xl border border-white/5 space-y-6">
            ...
            <div>
              ...
              <div className="relative">
                <input
                  ...
                  placeholder="0.00"
                  className="w-full bg-surface-container-lowest border border-white/10 rounded-lg px-4 py-3 text-on-surface font-headline text-2xl font-bold focus:border-primary focus:ring-1 focus:ring-primary"
                />
                <button
              ...
              <p className="text-xs text-secondary mt-2">
                {t("defi.walletBalance")}: {walletBalance.toLocaleString("en-US")} USDC
              </p>
            </div>

            {/* Duration Selector */}
            <div>
              <label className="block text-xs font-label uppercase tracking-widest text-secondary mb-3">
                Amount to Stake (USDC)
              </label>
                  ...
                  <button
                    ...
                    className={`py-2 rounded-lg font-headline font-bold text-sm transition-all ${
                      stakeDuration === days
                        ...
                        : "bg-surface-container-lowest text-on-surface border border-white/10 hover:border-primary/50"
                    }`}
                  >
                    {days} Days
                  </button>
            ...
            <button
              ...
              className="w-full py-4 rounded-xl bg-gradient-to-br from-primary to-on-primary-container text-on-primary font-headline font-bold text-lg hover:brightness-110 transition-all active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2"
            >
            ...
            <p className="text-[10px] text-secondary text-center">
              <span className="material-symbols-outlined text-xs align-middle">info</span> Staking involves smart
              contract risks. Past performance does not guarantee future results.
            </p>
          </div>
        </div>
      </main>
